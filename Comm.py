...
#SNR_dB_range=np.expand_dims(SNR_dB_range, axis=0)
...
for runIdx in range(0,NrSamples):
    ...
    #x = np.round(np.random.rand(2,2*K))-1;
...
with tf.variable_scope("embedding") as scope:
    if kk2 > 0:
      scope.reuse_variables() 
        ...
        #W2=tf.get_variable("embedding3",initializer=tf.random_normal([L,K,8*K],stddev=0.1),trainable=True,dtype=tf.float64)
...
def xk(z_hat,k,name='embedding'):
	...
	#return tf.matmul(W2[k-1],z_hat)+b2[k-1]
...
def model(x_f,f1_f,f2_f):
    ...
    pred=[]
   # prev_loss=0
    for dsIdx in range(0,batch_size):  ###to be changed
        #print(x_f)
        x = x_f[dsIdx]
        #print(x)
        f1 = f1_f[dsIdx] ##hty
...
with sess.as_default():
    ...
    for epoch in range(100,4000):
        ...
        file.close()
        loss_ep_t=0
        #id = "saved_models/model-epoch"+str(epoch)
        #os.mkdir(id)
